...
if "reset_pacchetto" in st.session_state and st.session_state.reset_pacchetto:
    ...
    # Reimposta numeri default a 0 (Chiavi v3 per forzare lo zero)
    if "num_ideali_v3" in st.session_state: st.session_state.num_ideali_v3 = 0
    if "num_proposte_v3" in st.session_state: st.session_state.num_proposte_v3 = 0
    if "num_accettate_v3" in st.session_state: st.session_state.num_accettate_v3 = 0
...
if scelta == "📝 NUOVA SCHEDA":
    ...
    with st.container(border=True):
        ...
        with col_ideali:
            # Nota: Ho cambiato la KEY in v3 per forzare il reset della memoria
            n_ideali = st.number_input("Sedute IDEALI:", value=0, min_value=0, key="num_ideali_v3")
        with col_proposte:
            # Nota: Ho cambiato la KEY in v3 per forzare il reset della memoria
            n_proposte = st.number_input("Sedute PROPOSTE:", value=0, min_value=0, key="num_proposte_v3")
        ...
        with col_conferma:
            # Di default prende il valore di n_proposte, ma parte da 0. Key v3
            n_accettate = st.number_input("Sedute ACCETTATE (Reali):", value=n_proposte, min_value=0, key="num_accettate_v3")
...
{dett}"""
    ...
    # Reimposta numeri default a 0 (Ho cambiato le KEY sotto per forzare lo zero)
    if "num_ideali_v2" in st.session_state: st.session_state.num_ideali_v2 = 0
    if "num_proposte_v2" in st.session_state: st.session_state.num_proposte_v2 = 0
    if "num_accettate_v2" in st.session_state: st.session_state.num_accettate_v2 = 0
    st.session_state.reset_pacchetto = False
...
if scelta == "📝 NUOVA SCHEDA":
    ...
    with st.container(border=True):
        ...
        col_ideali, col_proposte = st.columns(2)
        with col_ideali:
            # Nota: Ho cambiato la KEY in v2 per forzare il reset della memoria
            n_ideali = st.number_input("Sedute IDEALI:", value=0, min_value=0, key="num_ideali_v2")
        with col_proposte:
            # Nota: Ho cambiato la KEY in v2 per forzare il reset della memoria
            n_proposte = st.number_input("Sedute PROPOSTE:", value=0, min_value=0, key="num_proposte_v2")
        ...
        with col_conferma:
            # Di default prende il valore di n_proposte, ma parte da 0
            n_accettate = st.number_input("Sedute ACCETTATE (Reali):", value=n_proposte, min_value=0, key="num_accettate_v2")
